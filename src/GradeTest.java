...
// -------> TWO tests for inputs below and above the valid range for fromPercentage <-------
	...
	void testFromPercentageValid(int percentage) {
		assertDoesNotThrow(() -> Grade.fromPercentage(percentage), "Should not thrown out of boundary -1,0,100");
	}
...
// -------> FIVE tests for classify, using Classifications as equivalence classes <-------	
	...
	private static Stream<Arguments> testClassifyAsEQC() {
		return Stream.of(Arguments.of(2, Classification.First), Arguments.of(6, Classification.UpperSecond),
				Arguments.of(10, Classification.LowerSecond), Arguments.of(14, Classification.Third),
				Arguments.of(17, Classification.Fail),
				// additional tests added
				Arguments.of(18, Classification.Fail), Arguments.of(19, Classification.Fail),
...
// -------> TWENTY tests for fromPercentage, using each point in the 20-point scale as an equivalence class. <-------	
	...
	void testFromPrecentage(int percentage, int expectedGrade) {
		...
		assertEquals(expectedGrade, grade.getPoints(),
				"The grade should correctly convert from percentage to grade point.");
	}
